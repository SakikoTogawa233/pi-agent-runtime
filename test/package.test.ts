...
function dryRunPack(): PackEntry {
  ...
  return (JSON.parse(output) as PackEntry[])[0]!;
}

describe("packed runtime package", () => {
  it("contains declared runtime exports and no Pi resources or pi manifest", async () => {
    const manifest = JSON.parse(await readFile(join(repoRoot, "package.json"), "utf8")) as Record<string, unknown>;
    ...
    expect(paths.some((path) => /(^|\/)(extensions|skills|prompts|themes)(\/|$)/.test(path))).toBe(false);
    expect(paths.some((path) => /(?:^|\/)SKILL\.md$/.test(path))).toBe(false);
    expect(paths.some((path) => /(?:^|\/)pi\.json$/.test(path))).toBe(false);
    expect(paths.every((path) => /^(?:dist\/|README\.md$|LICENSE$|THIRD_PARTY_NOTICES\.md$|package\.json$)/.test(path))).toBe(true);
  });
  ...
  it("resolves published exports from production installs for both future consumer package names", async () => {
    ...
    const tarballName = (JSON.parse(packOutput) as PackEntry[])[0]!.filename;
    ...
    for (const name of consumerNames) {
      execFileSync(process.execPath, [join(root, name, "verify.mjs")], { cwd: join(root, name), stdio: "pipe" });
    }
