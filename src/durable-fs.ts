...
export interface DurableFileWriter {
  ...
  writePrivate(path: string, data: DurableData): Promise<void>;
...
async function writePrivateWithOperations(
  ...
  let handle: DurableWritableHandle;
  try {
    handle = await operations.openWritable(path, "wx", 0o600);
  } catch (error) {
    throwDurable(failure("open_file", path, error), [], path, undefined, false);
  }
  const result = await writeSyncClose(handle, path, data);
...
async function syncDirectoryAfterRename(
  ...
  try {
    ...
    throwDurable(
      failure("open_directory", directoryPath, error),
      [],
      targetPath,
      temporaryPath,
      true,
...
export function createDurableFileWriter(operations: DurableFileOperations): DurableFileWriter {
  return {
    ...
    async writePrivate(path: string, data: DurableData): Promise<void> {
      await writePrivateWithOperations(operations, path, data);
    },
...
export async function writePrivateFileDurable(path: string, data: DurableData): Promise<void> {
  await defaultWriter.writePrivate(path, data);
}
...
export async function replacePrivateFileDurable(path: string, data: DurableData): Promise<void> {
  await defaultWriter.replace(path, data);
}
...
export function assertPathContained(parent: string, child: string): void {
  if (!pathInside(parent, child)) {
    throw new Error(`Path must stay inside ${parent}: ${child}`);
  }
}
