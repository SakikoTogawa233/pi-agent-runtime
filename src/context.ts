...
import { canonicalJson, isJsonObject, type JsonObject } from "./canonical.js";
 ...
 *
 * Consumer artifact bytes are frozen against this implementation by ported
 * regression vectors.
 */
export const VISIBLE_CONVERSATION_TRANSFORM_ID = "visible-conversation-ledger-v2";

export const CONVERSATION_IMAGE_OMISSION_PREFIX = "[Image omitted from child text transcript: ";
...
export class UnsupportedConversationBlockError extends Error {
  ...
  constructor(label: string) {
    super(
      `agent runtime context projection encountered an unsupported conversation block: ${label}`,
    );
...
function ledgerLeafHash(index: number, record: OmittedEventRecord): Buffer {
  return createHash("sha256")
    .update(Buffer.from("pi-agent-runtime-ledger-leaf-v1\0", "utf8"))
...
function ledgerRootHash(leaves: readonly Buffer[]): string {
  const hash = createHash("sha256")
    .update(Buffer.from("pi-agent-runtime-ledger-root-v1\0", "utf8"))
 ...
 */
export function projectVisibleConversation(messages: readonly Message[]): ProjectedConversationV2 {
